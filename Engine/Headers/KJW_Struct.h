...
//#define _KJW_AI_CONTAINER_MAP;
//#define _KJW_AI_CONTAINER_UNORDED_MAP;
//#define _KJW_AI_CONTAINER_VECTOR;
#define _KJW_AI_CONTAINER_ARRAY
#define AI_ARRAY_SIZE_MAX 600
...
namespace Engine
{
	...
	struct JWVECTOR
	{
		~JWVECTOR()
		{
			Safe_Delete_Array(m_pVector);
		}
		T*	m_pVector = nullptr;
		int m_listCount = 0;
		int m_listSize = 0;
		
		bool	Push_Back(T pValue)
		{
			if (pValue == nullptr)
				return false;
			...
			if (m_listSize == 0)
				Reserve(5);
			else if (m_listCount >= m_listSize)
			{
				T* NewVector = new T[m_listSize * 2];
				for (int i = 0; i < m_listSize; ++i)
				{
					NewVector[i] = m_pVector[i];
				}
				for (int i = m_listSize; i < m_listSize * 2; ++i)
				{
					NewVector[i] = nullptr;
				}
		...
		FORCEINLINE T		Pop_Back()
		{
			...
			T pValue = m_pVector[m_listCount - 1];
			m_pVector[m_listCount - 1] = nullptr;
			--m_listCount;

			return pValue;
		}
		FORCEINLINE T		Get_Back()
		{
			if (m_listCount <= 0)
				return nullptr;

			return  m_pVector[m_listCount - 1];
		}
		FORCEINLINE T		Get(int iIndex)
		{
			if (m_listCount <= iIndex
				|| iIndex < 0
				|| iIndex >= m_listSize)
				return nullptr;
		...
		T		Pop(int iIndex)	//정렬 되지 않는다.
		{
			...
			T pValue = m_pVector[iIndex];

			m_pVector[iIndex] = m_pVector[m_listCount - 1];
			m_pVector[m_listCount - 1] = nullptr;
			--m_listCount;
		...
		FORCEINLINE int		Capacity() const
		{
			return m_listSize;
		}
		...
		JWVECTOR<T>& operator =(const JWVECTOR<T>& src)
		{
	...
	typedef struct FGenericPlatformMath
	{

		static FORCEINLINE D3DXVECTOR3 QuaternionsToEuler_Degree(D3DXQUATERNION QuaterRot)
		{
			D3DXVECTOR3 EulerAngle_Degree;

			double test = QuaterRot.x*QuaterRot.y + QuaterRot.z*QuaterRot.w;
			if (test > 0.499f) { // singularity at north pole  
				EulerAngle_Degree.y = 2 * atan2f(QuaterRot.x, QuaterRot.w);
				EulerAngle_Degree.z = D3DX_PI / 2;
				EulerAngle_Degree.x = 0;
			}
			else if (test < -0.499f) { // singularity at south pole  
				EulerAngle_Degree.y = -2 * atan2(QuaterRot.x, QuaterRot.w);
				EulerAngle_Degree.z = -D3DX_PI / 2;
				EulerAngle_Degree.x = 0;
			}
			else
			{
				double sqx = QuaterRot.x*QuaterRot.x;
				double sqy = QuaterRot.y*QuaterRot.y;
				double sqz = QuaterRot.z*QuaterRot.z;
				EulerAngle_Degree.y = D3DXToDegree(atan2f(2 * QuaterRot.y*QuaterRot.w - 2 * QuaterRot.x*QuaterRot.z, float(1 - 2 * sqy - 2 * sqz)));
				EulerAngle_Degree.z = D3DXToDegree(asinf(float(2 * test)));
				EulerAngle_Degree.x = D3DXToDegree(atan2f(2 * QuaterRot.x*QuaterRot.w - 2 * QuaterRot.y*QuaterRot.z, float(1 - 2 * sqx - 2 * sqz)));
			}

			return EulerAngle_Degree;
		}
		/**
		...
		*/
		...
		static FORCEINLINE float Atan2(float Y, float X) { return Atan2(Y, X); }
		...
		{
			return ((*(unsigned long long*)&A) >= (unsigned long long)0x8000000000000000); // Detects sign bit.
		}
		...
		//static CORE_API void SRandInit(int Seed);

		///** Returns the current seed for SRand(). */
		//static CORE_API int GetRandSeed();
		...
		*/
		static FORCEINLINE unsigned int FloorLog2(unsigned int Value)
		{
			...
			*/
			...
			int _forceinline method3(unsigned int v)
			{
		...
		* @return			Log2 of Value. 0 if Value is 0.
		*/
		static FORCEINLINE unsigned long long FloorLog2_64(unsigned long long Value)
		{
		...
		* @return the number of zeros before the first "on" bit
		*/
		static FORCEINLINE unsigned long long CountLeadingZeros64(unsigned long long Value)
		{
		...
		*/
		...
		static FORCEINLINE unsigned long long CeilLogTwo64(unsigned long long Arg)
		{
			long long Bitmask = ((long long)(CountLeadingZeros64(Arg) << 57)) >> 63;
		...
		*/
		//template< class T >
		//static FORCEINLINE T Min(const TArray<T>& Values, int* MinIndex = NULL)
		//{
		//	if (Values.Num() == 0)
		//	{
		//		if (MinIndex)
		...
		//*/
		//template< class T >
		//static FORCEINLINE T Max(const TArray<T>& Values, int* MaxIndex = NULL)
		//{
		//	if (Values.Num() == 0)
		//	{
		//		if (MaxIndex)
		...
		static FORCEINLINE int CountBits(unsigned long long Bits)
		{
