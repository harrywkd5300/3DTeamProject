...
CSword_UseTool::CSword_UseTool(LPDIRECT3DDEVICE9 pGraphicDev)
	: CEquipment(pGraphicDev)
{
#ifndef _FINAL
	Tag(L"CSword_UseTool", std::bind(&CSword_UseTool::Debug_Render, this));
#endif

}

HRESULT CSword_UseTool::Ready_GameObject(void)
{
...
_int CSword_UseTool::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bHide)
		return 0;
...
void CSword_UseTool::Render_GameObject(void)
{
...
HRESULT CSword_UseTool::Ready_Component(void)
{
	...
	// For.Mesh
	pComponent = m_pMeshCom = (Engine::CMesh_Static*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_TOOL, L"CMesh_Static_Sword");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CSword_UseTool::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CSword_UseTool * CSword_UseTool::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CSword_UseTool *	pInstance = new CSword_UseTool(pGraphicDev);
...
#ifndef _FINAL
void CSword_UseTool::Debug_Render()
{
	CDebug::GetInstance()->Add_DebugText("CSword_UseTool");

}
#endif
_ulong CSword_UseTool::Free(void)
{
	_ulong		dwRefCnt = 0;
