...
CSai_UseTool::CSai_UseTool(LPDIRECT3DDEVICE9 pGraphicDev)
	: CEquipment(pGraphicDev)
{
#ifndef _FINAL
	Tag(L"CSai_UseTool", std::bind(&CSai_UseTool::Debug_Render, this));
#endif

}

HRESULT CSai_UseTool::Ready_GameObject(void)
{
...
_int CSai_UseTool::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bHide)
		return 0;
...
void CSai_UseTool::Render_GameObject(void)
{
...
HRESULT CSai_UseTool::Ready_Component(void)
{
	...
	// For.Mesh
	pComponent = m_pMeshCom = (Engine::CMesh_Static*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_TOOL, L"CMesh_Static_Sai");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CSai_UseTool::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CSai_UseTool * CSai_UseTool::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CSai_UseTool *	pInstance = new CSai_UseTool(pGraphicDev);
...
#ifndef _FINAL
void CSai_UseTool::Debug_Render()
{
	CDebug::GetInstance()->Add_DebugText("CSai_UseTool");

}
#endif
_ulong CSai_UseTool::Free(void)
{
	_ulong		dwRefCnt = 0;
