...
CKnife_UseTool::CKnife_UseTool(LPDIRECT3DDEVICE9 pGraphicDev)
	: CEquipment(pGraphicDev)
{
#ifndef _FINAL
	Tag(L"CKnife_UseTool", std::bind(&CKnife_UseTool::Debug_Render, this));
#endif

}

HRESULT CKnife_UseTool::Ready_GameObject(void)
{
	if (FAILED(Ready_Component()))
		return E_FAIL;	
	
	m_pTransformCom->Set_Infomation(CTransform::INFO_ANGLE, &_vec3(90.f, 0.f, 90.f));
	m_pTransformCom->Set_Infomation(CTransform::INFO_POSITION, &_vec3(-8.f, 2.f, 0.f));

	/*
	m_pTransformCom->Rotation_Z(90.f, 1.f);
	m_pTransformCom->Rotation_X(90.f, 1.f);
...
_int CKnife_UseTool::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bHide)
		return 0;
...
void CKnife_UseTool::Render_GameObject(void)
{
...
HRESULT CKnife_UseTool::Ready_Component(void)
{
	...
	// For.Mesh
	pComponent = m_pMeshCom = (Engine::CMesh_Static*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_TOOL, L"CMesh_Static_Knife");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CKnife_UseTool::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CKnife_UseTool * CKnife_UseTool::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CKnife_UseTool *	pInstance = new CKnife_UseTool(pGraphicDev);
...
#ifndef _FINAL
void CKnife_UseTool::Debug_Render()
{
	CDebug::GetInstance()->Add_DebugText("CKnife_UseTool");

}
#endif
_ulong CKnife_UseTool::Free(void)
{
	_ulong		dwRefCnt = 0;
