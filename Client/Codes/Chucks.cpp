#include "stdafx.h"
#include "..\Headers\Chucks.h"

#include "Component_Manager.h"
#include "Object_Manager.h"

CChucks::CChucks(LPDIRECT3DDEVICE9 pGraphicDev)
	: CEquipment(pGraphicDev)
{
#ifndef _FINAL
	Tag(L"CChucks", std::bind(&CChucks::Debug_Render, this));
#endif

}

HRESULT CChucks::Ready_GameObject(void)
{
	if (FAILED(Ready_Component()))
		return E_FAIL;	
	
	m_pTransformCom->Rotation_X(90.0f, 1.f);
	

	return NOERROR;
}

_int CChucks::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bHide)
		return 0;
...
void CChucks::Render_GameObject(void)
{
...
HRESULT CChucks::Ready_Component(void)
{
	...
	// For.Mesh
	pComponent = m_pMeshCom = (Engine::CMesh_Static*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"CMesh_Static_Chucks");
	if (nullptr == pComponent)
		return E_FAIL;
...
void CChucks::Collision_ToStone(void)
{
...
HRESULT CChucks::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CChucks * CChucks::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CChucks *	pInstance = new CChucks(pGraphicDev);
...
#ifndef _FINAL
void CChucks::Debug_Render()
{
	CDebug::GetInstance()->Add_DebugText("CChucks");

}
#endif
_ulong CChucks::Free(void)
{
	_ulong		dwRefCnt = 0;
