#include "stdafx.h"
#include "..\Headers\Knife.h"

#include "Component_Manager.h"
#include "Object_Manager.h"

CKnife::CKnife(LPDIRECT3DDEVICE9 pGraphicDev)
	: CEquipment(pGraphicDev)
{
#ifndef _FINAL
	Tag(L"CKnife", std::bind(&CKnife::Debug_Render, this));
#endif

}

HRESULT CKnife::Ready_GameObject(void)
{
...
_int CKnife::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bHide)
		return 0;
...
void CKnife::Render_GameObject(void)
{
...
HRESULT CKnife::Ready_Component(void)
{
	...
	// For.Mesh
	pComponent = m_pMeshCom = (Engine::CMesh_Static*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"CMesh_Static_Knife");
	if (nullptr == pComponent)
		return E_FAIL;
...
void CKnife::Collision_ToStone(void)
{
...
HRESULT CKnife::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CKnife * CKnife::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CKnife *	pInstance = new CKnife(pGraphicDev);
...
#ifndef _FINAL
void CKnife::Debug_Render()
{
	CDebug::GetInstance()->Add_DebugText("CKnife");

}
#endif
_ulong CKnife::Free(void)
{
	_ulong		dwRefCnt = 0;
